...
const RenameModal: React.FC<RenameModalProps> = ({ onClose, onRename, currentName, isFolder }) => {
      ...
      <div className="bg-dark-800 rounded-lg shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-dark-700">
          <h2 className="text-xl font-semibold text-dark-100">
            <TranslatedText text={`Rename ${isFolder ? 'Folder' : 'File'}`} />
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4">
          <div className="space-y-4">
            <div>
              <label htmlFor="newName" className="block text-sm font-medium text-dark-300">
                <TranslatedText text={`New ${isFolder ? 'Folder' : 'File'} Name`} />
              </label>
              <input
                type="text"
                id="newName"
            ...
            >
              {isRenaming ? (
                <div className="flex items-center space-x-2">
                  ...
                  <span><TranslatedText text="Renaming..." /></span>
                </div>
              ) : (
